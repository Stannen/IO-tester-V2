...
REM === Pas dit pad aan naar jouw projectlocatie ===
...
python -m PyInstaller --onefile --name %APPNAME% "%ENTRY%"
if errorlevel 1 (
...
if exist "%PROJECT_PATH%\dist\%APPNAME%.exe" (
    copy "%PROJECT_PATH%\dist\%APPNAME%.exe" "%DIST_DIR%\"
) else (
...
for %%d in (%EXTRA_DIRS%) do (
    if exist "%PROJECT_PATH%\%%d" (
        echo - Kopieer %%d...
        xcopy "%PROJECT_PATH%\%%d" "%DIST_DIR%\%%d" /E /I /Y >nul
    ) else (
